...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='i3wm-themer by Stavros Grigoriou')
    parser.add_argument('-c', '--config', type=str,
                        required=True, help='Load config file')
    parser.add_argument('-b', '--backup', type=str, help='Backup files')
    parser.add_argument('-i', '--install', type=str,
                        help='Install i3wmthemer\'s default configuration files')
    parser.add_argument('-l', '--load', type=str,
                        help='Load theme from JSON file')
    ...
    if args.load != None:
        ...
        com.refresh_all(
            configuration['xresources'], configuration['wallpaper-path'], jfile['wallpaper'])
        exit(0)
