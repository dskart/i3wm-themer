...
def replace_wallpaper(configuration, json_file):
    ...
    if(fileu.locate_file(configuration['nitrogen-config'])):
        ...
        if 'wallpaper' in json_file:
            ...
            rl.replace_line(configuration['nitrogen-config'], 'file',
                            'file= '+configuration['wallpaper-path']+wallpaper)
            new_file = '/home/raphaelvh/Theme/i3wm-themer/src/wallpapers/'+wallpaper
            try:
                copyfile(
                    new_file, configuration['wallpaper-path']+wallpaper)
                prnt.prnt('-s', 'Installed the new file successfully!')
                return True
            except:
                prnt.prnt('-f', 'Failed to install the new file!')
                prnt.prnt('-f', new_file)
                return False
    else:
